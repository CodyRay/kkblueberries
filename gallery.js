...
if (!directory) {
  throw new Error(
    'Please provide the directory containing the images and the directory.yml file'
  )
}
  ...
  items
    ...
    .forEach((item) => {
      if (gallery.photos.filter((p) => p.file === item).length === 0) {
        throw new Error(
          item + " exists in the directory, but it isn't in the *.yaml"
        )
      }
    })
})
...
var proms = gallery.photos.map((p) => {
  ...
  return Object.keys(gallery.sizes).map((size) => {
    ...
    if (width && height) {
      // We are getting a specific aspect ratio
      return smartcrop
        .crop(image, { width: width, height: height })
...
Promise.all(_.flatten(proms), originalProms)
  .then(() => {
    ...
    gallery.photos.forEach((p) => {
      ...
      function importFile(file) {
        fs.writeSync(
          fileJs,
          `import ${_.camelCase(file)} from './${file.replace(
            path.sep,
            '/'
          )}'\n`
        )
    ...
    gallery.photos.forEach((p) => {
      key++
      var sizes = Object.keys(p.sizes).map(
        (s) => `${s}: ${_.camelCase(p.sizes[s])}`
      )
      var obj = [
        `key: ${key}`,
        `file: '${p.file}'`,
